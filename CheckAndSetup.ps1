...
if ($fail -eq 0){
    ...
    Invoke-WebRequest -URI https://github.com/andretapxure/zenerScripts/blob/master/BackupOne.ps1?raw=True -OutFile $pathRclone/BackupOne.ps1
    Write-Host ('Fazendo Download do arquivo BackupAll') -ForegroundColor Green
    Invoke-WebRequest -URI https://github.com/andretapxure/zenerScripts/blob/master/BackupAll.ps1?raw=True -OutFile $pathRclone/BackupAll.ps1
    Write-Host ('Fazendo Download do arquivo Filters.txt') -ForegroundColor Green
    Invoke-WebRequest -URI https://github.com/andretapxure/zenerScripts/blob/master/filters.txt?raw=True -OutFile $pathRclone/filters.txt
